    ...
    <div style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      {orig && (
        <div style={{ background: "#1c1846", borderRadius: 10, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 11, color: "#6f6798", marginBottom: 12, lineHeight: 1.5 }}>
            Прикидка по {CANDIDATE_CITIES.length} точкам по всему миру — не только популярные столицы, но и менее очевидные места на каждом континенте: где благоприятные планеты попадают в сильные дома (1,4,5,7,9,10), а трудные — в спокойные (6,8,12), как это влияет на дом текущей махадаши, и отдельно — насколько силён управитель Асцендента (Лагна-лорд) в этой точке. Список большой, но конечный (пересчитать буквально каждую точку планеты через API нереально) — эвристика поверх карты, не классическая методика подбора места. Можно сузить поиск до конкретной части света, либо проверить вручную любой город или координаты ниже.
          </div>
